...
extern unsigned input[3];     // Array of serial inputs
extern float acceleration[3]; // Array of accelrations
extern int TDRE_BIT;

// Parameters to convert float accelerations to string
...
void read_data(void) {
  ...
  input[0] = ATD1DR0;       // x input

  input[1] = ATD1DR1;       // y input

  input[2] = ATD1DR2;       // z input
  
}


// function to intialise ADC  
void init_ADC(void) {
...
// function to calculate acceleration based on the data received from the sensor
void calc_acc(void) {
...
void float2str(float n) {
  ...
  /* Saved string is opposite direction to real value */
  ...
  dVal = (int)n * absolute;    // Integer parts
  dec = (int)(n * 100 * absolute) % 100; // Decimal parts
  
  // Save one by one for decimal parts to str[]
  str[str_index++] = (dec % 10) + '0';
  str[str_index++] = (dec / 10) + '0';  
  str[str_index++] = '.';
  
  // Save one by one for integer parts to str[]
  while (dVal > 0) {
